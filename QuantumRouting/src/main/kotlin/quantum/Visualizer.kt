...
class MyCanvas(val algorithm: MyfirstTest) : JPanel() {
  ...
  internal fun drawTopo(g: Graphics2D) = try {
    ...
    topo.nodes.forEach { n ->
      ...
      g.color = if (algorithm.srcDstPairs.any { it.src == n }) Color.MAGENTA else if (algorithm.srcDstPairs.any { it.dst == n }) Color.BLUE else Color.BLACK
...
class Visualizer(algorithm: MyfirstTest) : JFrame() {
  var clicked = true
